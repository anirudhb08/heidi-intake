...
Called once per medication before the agent reads it back. Four outcomes:
  ...
  close misspelling, one   -> "the closest is X, is that right?", once
...
class LookupTool:
    ...
    async def lookup_medication(
        ...
        if not candidates:
            ...
            # One drug: exact, or a close misspelling with no sound-alike (a fuzzy hit
            # that has sound-alikes arrives here with them added, so it is ambiguous).
            canon = next(iter(candidates))
            sound_alike = sorted(self.formulary.canon(x) for x in self.formulary.sound_alikes([canon]))
            result = {
                "matched": True,
                "recognised_name": canon,
                "sound_alike": sound_alike,
                "confirm_recommended": fuzzy or bool(sound_alike),
            }
            if sound_alike:
                result["instruction"] = (
                ...
                result["instruction"] = (
                    f"'{spoken_name}' was not an exact match; the closest medication is {canon}. "
                    f"Read back \"{canon}\" and ask once if that is right. If they confirm, record {canon}; "
                    "if not, record the name exactly as they said it."
                )
        else:
